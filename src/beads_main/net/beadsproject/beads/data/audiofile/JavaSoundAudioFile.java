 ...
 * 
 * KNOWN ISSUE: Certain .wav files will be not be able to be loaded.
 * This is due to mp3spi recognizing them incorrectly as mp3s.
 * This will hopefull be fixed in the future, but until then 
 * resaving your .wavs with a different audio util may help.
 * 
 * @author ben
 */
public class JavaSoundAudioFile extends AudioFile {
	 ...
	 */
	public JavaSoundAudioFile(String filename) throws IOException, UnsupportedAudioFileException {
		this(filename,-1);
	}
	 ...
	 * @throws javax.sou 
	 */
	public JavaSoundAudioFile(String filename, int bufferSize) throws IOException, UnsupportedAudioFileException {
		...
		if (!name.endsWith(".mp3"))
		{
			...
			{
				throw(new UnsupportedAudioFileException("Cannot read \"" + name + "\". " +
	...
	public JavaSoundAudioFile(InputStream stream) throws IOException, UnsupportedAudioFileException {
		this(stream, -1);
	}
	
	public JavaSoundAudioFile(InputStream stream, int bufferSize) throws IOException, UnsupportedAudioFileException {
	...
	public void open() throws UnsupportedAudioFileException, IOException
	{
		...
		decodedFormat = new javax.sound.sampled.AudioFormat(javax.sound.sampled.AudioFormat.Encoding.PCM_SIGNED,
		...
		else
		{
			...
			setLength(1000.f * decodedStream.getFrameLength() / decodedFormat.getSampleRate());
		}
		...
		if (trace) System.err.printf("AudioFile \"%s\" open end\n",name);
	}
...
//		if (file.exists())
	...
	{	
		if (buffer.length!=getNumChannels() || buffer[0].length==0) return 0;			
		...
		float[] floatbuf = new float[buffer[0].length*getNumChannels()*numFramesJustRead];
		AudioUtils.byteToFloat(floatbuf, byteBuffer, decodedFormat.isBigEndian(),numFramesJustRead*getNumChannels());    		
		AudioUtils.deinterleave(floatbuf,getNumChannels(),numFramesJustRead,buffer);
